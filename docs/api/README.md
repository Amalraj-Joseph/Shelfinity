# Project Documentation

## Table of Contents

1. [SRS (Software Requirement Specification)](SRS.md)
2. [As-Is Analysis](As_is.md)
3. [User Stories](User_stories.md)
4. [Architecture](Architecture.md)
6. [Flow Chart](Flow_chart.md)
7. [OpenAPI Spec (As YAML) - api.yaml](api.yaml)
8. [OpenAPI Spec (AS HTML) - api.html](api.html)


## Library Management System : Layered Architecture

<img width="1323" alt="Screenshot 2025-04-15 at 5 15 12 PM" src="https://github.com/user-attachments/assets/c830c49a-cc0b-4313-858d-d06ddb04b855" />



